...
["", "systemsolver.dart", , K, {
  "^": "",
  main: [function() {
    var t1 = J.get$onChange$x(document.querySelector("#equationNum"));
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(K.updateSystem$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    t1 = J.get$onClick$x(document.querySelector("#submit"));
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(K.pressSubmit$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
  }, "call$0", "main$closure", 0, 0, 2],
  pressHere: [function(e) {
    var matrix, i, t1, list, j;
    matrix = H.setRuntimeTypeInfo([], [[P.List, P.$int]]);
    i = 0;
    while (true) {
      t1 = H.Primitives_parseInt($.get$numEquation().value, null, null);
      if (typeof t1 !== "number")
        return H.iae(t1);
      if (!(i < t1))
        break;
      ...
      while (true) {
        ...
        if (i >= 4)
          return H.ioore(t1, i);
        ++j;
        t1 = "#" + t1[i] + C.JSInt_methods.toString$0(j);
        t1 = H.interceptedTypeCast(document.querySelector(t1), "$isTextInputElement").value;
        if (t1.length > 0)
          list.push(H.Primitives_parseInt(t1, null, null));
        else
          list.push(0);
      }
      ++i;
      t1 = "#C" + C.JSInt_methods.toString$0(i);
      t1 = H.interceptedTypeCast(document.querySelector(t1), "$isTextInputElement").value;
      if (t1.length > 0)
        list.push(H.Primitives_parseInt(t1, null, null));
      else
    ...
    K.cramersRule(matrix);
  }, "call$1", "pressHere$closure", 2, 0, 8],
  rowEchelon: function(matrix) {
    var i, j, j0, t1, index, nonZero, k, switchList, l, t2, m, t3, t4, n, system, o, p;
    for (i = 0; i < matrix.length - 1; i = j)
      for (j = i + 1, j0 = j; j0 < matrix.length; ++j0) {
        t1 = matrix[j0];
        if (i >= t1.length)
          return H.ioore(t1, i);
        if (!J.$eq(t1[i], 0)) {
          ...
          while (true) {
            ...
            if (!J.$eq(t1[i], 0)) {
              if (k >= matrix.length)
                return H.ioore(matrix, k);
              t1 = matrix[k];
              if (i >= t1.length)
                return H.ioore(t1, i);
              nonZero = J.toDouble$0$n(t1[i]);
              index = k;
              break;
            }
            ++k;
          ...
          if (nonZero === 0) {
            switchList = [];
            switchList.$builtinTypeInfo = [P.$int];
            l = 0;
            while (true) {
              t1 = matrix.length;
              if (j0 >= t1)
                return H.ioore(matrix, j0);
              t2 = matrix[j0];
              if (!(l < t2.length))
                break;
              switchList.push(t2[l]);
              ++l;
            }
            for (t2 = switchList.length, m = 0; t3 = matrix[j0], m < t3.length; ++m) {
              if (i >= t1)
            ...
            if (i >= t1.length)
              return H.ioore(t1, i);
            nonZero = J.toDouble$0$n(t1[i]) / nonZero;
            n = 0;
            while (true) {
              t1 = matrix.length;
              if (j0 >= t1)
              ...
              t3 = t2[n];
              if (index >= t1)
                return H.ioore(matrix, index);
              t1 = matrix[index];
              if (n >= t1.length)
                return H.ioore(t1, n);
              t1 = J.$sub$n(t3, C.JSNumber_methods.toInt$0(J.toDouble$0$n(t1[n]) * nonZero));
              if (n >= t2.length)
                return H.ioore(t2, n);
              t2[n] = t1;
              ++n;
    ...
    t1.set$innerHtml(system, "");
    for (o = 0; o < matrix.length; ++o) {
      p = 0;
      while (true) {
        if (o >= matrix.length)
          return H.ioore(matrix, o);
        t2 = matrix[o];
        if (!(p < t2.length))
          break;
        t2 = J.toString$0(t2[p]);
        system.appendChild(document.createTextNode(t2));
        system.appendChild(document.createTextNode(" "));
        ++p;
      }
      t1.insertAdjacentHtml$2(system, "beforeend", "<br>");
    }
  },
  cramersRule: function(matrix) {
    var $D, system, t1, i, tempMatrix, j, list, k, t2, t3, Dn, subscript;
    $D = J.toDouble$0$n(K.findDeterminant(matrix));
    if ($D !== 0) {
      system = document.querySelector("#system");
      t1 = J.getInterceptor$x(system);
      t1.set$innerHtml(system, "");
      for (i = 0; i < matrix.length; ++i) {
        tempMatrix = [];
        tempMatrix.$builtinTypeInfo = [[P.List, P.$int]];
        for (j = 0; j < matrix.length; ++j) {
          ...
          tempMatrix.push(list);
        }
        Dn = J.toDouble$0$n(K.findDeterminant(tempMatrix));
        if (Dn !== 0) {
          system.appendChild(document.createTextNode("x"));
          subscript = document.createElement("sub", null);
          t2 = C.JSInt_methods.toString$0(i + 1);
          subscript.toString;
          subscript.appendChild(document.createTextNode(t2));
          system.appendChild(subscript);
          t2 = " = " + C.JSNumber_methods.toString$0(Dn / $D);
          system.appendChild(document.createTextNode(t2));
          t1.insertAdjacentHtml$2(system, "beforeend", "<br>");
        } else
          K.rowEchelon(matrix);
  ...
  findDeterminant: function(matrix) {
    var t1, currentDeterminant, i, newMatrix, first, j, list, k, t2, t3;
    t1 = matrix.length;
    if (t1 !== 2) {
      for (currentDeterminant = 0, i = 0; i < matrix.length; ++i) {
        newMatrix = [];
        newMatrix.$builtinTypeInfo = [[P.List, P.$int]];
        for (first = true, j = 0; j < matrix.length; ++j)
          if (i !== j) {
            list = [];
            list.$builtinTypeInfo = [P.$int];
            for (k = 0; t1 = matrix.length, k < t1 - 1; ++k)
              if (first) {
                if (i >= t1)
                  return H.ioore(matrix, i);
                t1 = matrix[i];
        ...
        t1 = K.findDeterminant(newMatrix);
        if (typeof t1 !== "number")
          return H.iae(t1);
        currentDeterminant += t1;
      }
      return currentDeterminant;
    } else {
      if (0 >= t1)
        return H.ioore(matrix, 0);
      t2 = matrix[0];
      if (0 >= t2.length)
        return H.ioore(t2, 0);
      t2 = t2[0];
      ...
      if (0 >= t2.length)
        return H.ioore(t2, 0);
      return J.$sub$n(t1, J.$mul$ns(t3, t2[0]));
    }
  },
  pressSubmit: [function(e) {
    var here, t1;
    if (!J.$eq(H.Primitives_parseInt($.get$numEquation().value, null, null), 0)) {
      document.querySelector("#equationNum").setAttribute("disabled", "true");
      document.querySelector("#submit").hidden = true;
      K.formRealSystem($.get$numEquation().value);
      document.querySelector("#check").hidden = true;
      here = document.createElement("button", null);
      here.id = "here";
      here.textContent = "here";
      t1 = document.querySelector("#system");
      t1.toString;
      t1.appendChild(document.createTextNode("Enter the values of your system then click "));
      document.querySelector("#system").appendChild(here);
      t1 = J.get$onClick$x(document.querySelector("#here"));
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(K.pressHere$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    }
  }, "call$1", "pressSubmit$closure", 2, 0, 8],
  updateSystem: [function(e) {
    if (!J.$eq(H.Primitives_parseInt($.get$numEquation().value, null, null), 0)) {
      document.querySelector("#equations").hidden = false;
      K.formSampleSystem($.get$numEquation().value);
      document.querySelector("#submit").hidden = false;
    }
  }, "call$1", "updateSystem$closure", 2, 0, 8],
  formSampleSystem: function(equations) {
    var equationList, i, t1, equation, j, t2, subscriptA, subscriptX, subscriptC;
    equationList = document.querySelector("#system");
    J.set$innerHtml$x(equationList, "");
    i = 1;
    while (true) {
      t1 = H.Primitives_parseInt(equations, null, null);
      if (typeof t1 !== "number")
        return H.iae(t1);
      if (!(i <= t1))
        break;
      equation = document.createElement("p", null);
      t1 = i - 1;
      j = 1;
      while (true) {
        t2 = H.Primitives_parseInt(equations, null, null);
        if (typeof t2 !== "number")
          return H.iae(t2);
        if (!(j <= t2))
          break;
        subscriptA = document.createElement("sub", null);
        subscriptX = document.createElement("sub", null);
        subscriptC = document.createElement("sub", null);
        t2 = $.get$letters();
        if (t1 >= 4)
          return H.ioore(t2, t1);
        t2 = t2[t1];
        equation.toString;
        equation.appendChild(document.createTextNode(t2));
        t2 = C.JSInt_methods.toString$0(j);
        subscriptA.toString;
        subscriptA.appendChild(document.createTextNode(t2));
        equation.appendChild(subscriptA);
        equation.appendChild(document.createTextNode("x"));
        t2 = C.JSInt_methods.toString$0(j);
        subscriptX.toString;
        subscriptX.appendChild(document.createTextNode(t2));
        equation.appendChild(subscriptX);
        t2 = H.Primitives_parseInt(equations, null, null);
        if (typeof t2 !== "number")
          return H.iae(t2);
        if (j < t2)
          ...
          equation.appendChild(document.createTextNode(" = C"));
          t2 = C.JSInt_methods.toString$0(i);
          subscriptC.toString;
          subscriptC.appendChild(document.createTextNode(t2));
          equation.appendChild(subscriptC);
        }
        ++j;
      }
      equationList.appendChild(equation);
      ++i;
    }
  },
  formRealSystem: function(equations) {
    var equationList, i, t1, equation, j, t2, subscriptX, coeff, t3, $C;
    equationList = document.querySelector("#system");
    J.set$innerHtml$x(equationList, "");
    i = 1;
    while (true) {
      ...
      while (true) {
        ...
        if (!(j <= t2))
          break;
        subscriptX = document.createElement("sub", null);
        coeff = document.createElement("input", null);
        t2 = J.getInterceptor$x(coeff);
        t2.set$type(coeff, "text");
        t3 = $.get$letters();
        if (t1 >= 4)
          return H.ioore(t3, t1);
        coeff.id = t3[t1] + C.JSInt_methods.toString$0(j);
        t2.set$size(coeff, 4);
        equation.appendChild(coeff);
        equation.appendChild(document.createTextNode("x"));
        t2 = C.JSInt_methods.toString$0(j);
        subscriptX.toString;
        subscriptX.appendChild(document.createTextNode(t2));
        ...
        if (j < t2)
          ...
          equation.appendChild(document.createTextNode(" = "));
          $C = document.createElement("input", null);
          t2 = J.getInterceptor$x($C);
          t2.set$type($C, "text");
          $C.id = "C" + C.JSInt_methods.toString$0(i);
          t2.set$size($C, 3);
...
Isolate.$lazy($, "numEquation", "numEquation", "get$numEquation", function() {
  return H.interceptedTypeCast(document.getElementById("equationNum"), "$isSelectElement");
});
Isolate.$lazy($, "letters", "letters", "get$letters", function() {
  return ["a", "b", "c", "d"];
});
...
//# sourceMappingURL=systemsolver.dart.js.map
